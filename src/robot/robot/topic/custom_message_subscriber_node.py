...
class CustomMessageSubscriberNode(Node):
    def __init__(self):
        # Node 초기화 및 노드 이름 등록
        super().__init__('custom_message_subscriber_node')
...
def main(args=None):
    ...
    node = CustomMessageSubscriberNode()                               # 노드 생성
    cv2.namedWindow('custom_message_subscriber', cv2.WINDOW_NORMAL)    # 표시할 윈도우 생성
    try:
        while rclpy.ok():   # ROS 2 런타임이 현재 정상 동작 중인지 확인
            ...
            if node._latest_frame is not None:
                cv2.imshow('custom_message_subscriber', node._latest_frame)
